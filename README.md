...
PowerShellを開かず、変更をGitHubと公開ページへ反映できます。

1. GitHub DesktopでこのリポジトリをCloneします。ZIPのダウンロードではなく、必ずCloneを使用してください。
2. Cloneしたフォルダー内のHTML・CSS・JavaScript・画像を編集して保存します。
3. フォルダー内の `更新を反映.cmd` をダブルクリックします。
4. 変更が自動的にコミットされ、`main` ブランチへpushされます。
5. GitHub Pagesが通常1分ほどで自動更新されます。

このボタンは、Gitに登録されていないものを含め、`.gitignore` の対象外となる変更をまとめて反映します。編集中のファイルがないことを確認してから実行してください。エラーが表示された場合は、画面を閉じる前に内容を確認してください。

## 10. 画面画像をドラッグ＆ドロップで追加・削除する方法

1. 公開中の画面遷移図ビューアーで、編集したい画面ノードをクリックします。
2. 右側の「画像保存先を接続」を押します。
3. GitHub DesktopでCloneした `biyou-clinic-screen-flow` フォルダーを選択します。
4. 右側の追加エリアへPNG・JPG・WebP・GIF画像をドラッグ＆ドロップします。複数枚を同時に追加できます。
5. 既存画像を消す場合は、画像下部の「表示中の画像を削除」を押し、確認画面で削除します。
6. Cloneしたフォルダー内の `更新を反映.cmd` をダブルクリックすると、画像と設定がGitHubへpushされ、公開ページも自動更新されます。

ブラウザの安全制限により、編集を始めるときはClone済みフォルダーの選択が必要です。この編集機能はChromeまたはEdgeで使用してください。画像の追加・削除情報は `config/image-overrides.json` に保存されます。
